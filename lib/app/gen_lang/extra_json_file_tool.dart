...
Map<String, FileSystemEntity> getValidStringFileMap(files) {
  var validMap = <String, FileSystemEntity>{};
...
FileSystemEntity? getDefaultTemplateLang(
  ...
  var defaultFile = validFilesMap[lang];
  if (defaultFile != null) {
    return defaultFile;
  }

  for (var defaultFile in validFilesMap.values) {
    return defaultFile;
  }
...
Future<Map<String, Message>> generateJsonKeyMessageMap(File jsonFile) async {
  ...
  var normalizedJson = normalizedSpecialCharacters(json);

  Map<String, dynamic> jsonMap = jsonDecode(normalizedJson!);

  var keyMap = <String, Message>{};

  for (var jsonEntry in jsonMap.entries) {
    var k = jsonEntry.key;
    String v = jsonEntry.value;

    var _messageKey = getMessageKey(k);
    var _message;
